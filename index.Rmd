...
### **Popularity Contest**<br>Track Index
...
unnested = bach_prelude_playlist_features %>%
  select(album_tracks, album_artist_label) %>%
  unnest(album_tracks) %>% 
...
In this analysis, the popularity of the track is compared to the tracks that follow it. In each of these cases, this is the first prelude, and is usually the first track on the album, with the highest popularity. There seem to be two signatures at play:  
1. Schiff, Kempff, Cloud, and Beausejour have a C-major prelude that is much more popular than their respective album means.  
2. Lang Lang, Pinnock, and Robles seem to be less in general, and do not see a large boost to their first track's popularity.  
...
1. Artists who are famous for playing classical music (eg: Sir Andras Schiff), are much more popular than other artists.  
2. Compilations (eg: classical boost!) are much less popular than original albums of a single musical work.  
